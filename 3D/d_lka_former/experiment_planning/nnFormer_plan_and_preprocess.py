...
from d_lka_former.experiment_planning.DatasetAnalyzer import DatasetAnalyzer
from d_lka_former.experiment_planning.utils import crop
from d_lka_former.paths import *
import shutil
from d_lka_former.utilities.task_name_id_conversion import convert_id_to_task_name
from d_lka_former.preprocessing.sanity_checks import verify_dataset_integrity
from d_lka_former.training.model_restore import recursive_find_python_class
...
def main():
    ...
    search_in = join(d_lka_former.__path__[0], "experiment_planning")

    if planner_name3d is not None:
        planner_3d = recursive_find_python_class([search_in], planner_name3d, current_module="d_lka_former.experiment_planning")
        if planner_3d is None:
            raise RuntimeError("Could not find the Planner class %s. Make sure it is located somewhere in "
                               "d_lka_former.experiment_planning" % planner_name3d)
    else:
        planner_3d = None

    if planner_name2d is not None:
        planner_2d = recursive_find_python_class([search_in], planner_name2d, current_module="d_lka_former.experiment_planning")
        if planner_2d is None:
            raise RuntimeError("Could not find the Planner class %s. Make sure it is located somewhere in "
                               "d_lka_former.experiment_planning" % planner_name2d)
    else:
        planner_2d = None
