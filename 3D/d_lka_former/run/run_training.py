...
def main():
    ...
    parser.add_argument('--trans_block', default='TransformerBlock', type=str, help='The chosen Transformerblock. There are several different to choose from.')

    parser.add_argument('--depths', required=False, default=3, type=int, help='Depth of the Transformerblocks per stage.')

    parser.add_argument('--skip_connections', required=False, default=4, type=int, help='Number of skip connections in the network.')
    parser.add_argument('--seed', type=int, default=12345, help='Seed for reproducing the training.')
    ...
    if args.network_trainer == 'd_lka_former_trainer_synapse':
        module = importlib.import_module('d_lka_former.network_architecture.synapse.transformerblock')
    elif args.network_trainer == 'd_lka_former_trainer_acdc':
        module = importlib.import_module('d_lka_former.network_architecture.acdc.transformerblock')
    ...
    if args.skip_connections == 4:
        ...
        raise RuntimeError("Number of skip connections must be between 0 and 4, but it is: {}".format(args.skip_connections))
    ...
    if trainer_class is None:
        raise RuntimeError("Could not find trainer class in d_lka_former.training.network_training")


    trainer = trainer_class(plans_file, fold, output_folder=output_folder_name, dataset_directory=dataset_directory,
                            batch_dice=batch_dice, stage=stage, unpack_data=decompress_data,
                            deterministic=deterministic, fp16=run_mixed_precision, trans_block=trans_block_class, depths=depths, skip_connections=skip_connections, seed=args.seed)
        ...
        if not validation_only:
            if args.continue_training:
                ...
                try:
                    ...
                    print("No model found to continue training. Starting from scratch...")
                    pass
            else:
