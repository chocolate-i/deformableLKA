...
my_output_identifier = "d_lka_former"
default_plans_identifier = "d_lka_former_Plansv2.1"
default_data_identifier = 'd_lka_former_Data_plans_v2.1'
default_trainer = "d_lka_former_trainer_synapse"

"""
PLEASE READ paths.md FOR INFORMATION TO HOW TO SET THIS UP
"""

base = os.environ['d_lka_former_raw_data_base'] if "d_lka_former_raw_data_base" in os.environ.keys() else None
preprocessing_output_dir = os.environ['d_lka_former_preprocessed'] if "d_lka_former_preprocessed" in os.environ.keys() else None
network_training_output_dir_base = os.path.join(os.environ['RESULTS_FOLDER']) if "RESULTS_FOLDER" in os.environ.keys() else None

if base is not None:
    nnFormer_raw_data = join(base, "d_lka_former_raw_data")
    nnFormer_cropped_data = join(base, "d_lka_former_cropped_data")
    maybe_mkdir_p(nnFormer_raw_data)
    maybe_mkdir_p(nnFormer_cropped_data)
else:
    print("d_lka_former_raw_data_base is not defined and model can only be used on data for which preprocessed files "
...
if preprocessing_output_dir is not None:
    ...
    print("d_lka_former_preprocessed is not defined and model can not be used for preprocessing "
