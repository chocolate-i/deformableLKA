...
from networks.d_lka_former.transformerblock import TransformerBlock, TransformerBlock_3D_LKA, TransformerBlock_LKA_Channel, TransformerBlock_SE, TransformerBlock_Deform_LKA_Channel, TransformerBlock_Deform_LKA_Channel_sequential, TransformerBlock_3D_LKA_3D_conv, TransformerBlock_LKA_Channel_norm, TransformerBlock_Deform_LKA_Spatial_sequential, TransformerBlock_Deform_LKA_Spatial, TransformerBlock_3D_single_deform_LKA
...
class D_LKA_NetEncoder(nn.Module):
    def __init__(self, input_size=[32 * 32 * 32, 16 * 16 * 16, 8 * 8 * 8, 4 * 4 * 4],dims=[32, 64, 128, 256],
        ...
        stem_layer = nn.Sequential(
            get_conv_layer(spatial_dims, in_channels, dims[0], kernel_size=(2, 2, 2), stride=(2, 2, 2), #kernel_size=(2, 4, 4), stride=(2, 4, 4),
        ...
        for i in range(4):
            ...
            for j in range(depths[i]):
                stage_blocks.append(trans_block(input_size=input_size[i], hidden_size=dims[i],  proj_size=proj_size[i], num_heads=num_heads,
                                     dropout_rate=transformer_dropout_rate, pos_embed=True)) ## LEON HERE, change pos_embed back to True
        ...
        if conv_decoder == True:
            ...
            for j in range(depth):
                stage_blocks.append(trans_block(input_size=out_size, hidden_size= out_channels, proj_size=proj_size, num_heads=num_heads,
                                                     dropout_rate=0.15, pos_embed=True)) ## LEON here, change pos_embed to True
