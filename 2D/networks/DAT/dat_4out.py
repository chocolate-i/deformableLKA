...
from networks.DAT.dat_blocks import *
...
class DAT(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.patch_proj(x)
        #print("Projection shape: {}".format(x.shape))
        positions = []
        references = []
        outs = []
        for i in range(4):
            ...
            outs.append(x)
            #print("Stage {} x shape: {}".format(i,x.shape))
            if i < 3:
        ...
        # Here is just classification stuff going on    
        #x = self.cls_norm(x)
        #x = F.adaptive_avg_pool2d(x, 1)
        #x = torch.flatten(x, 1)
        #x = self.cls_head(x)
        
        return outs #x, positions, references
