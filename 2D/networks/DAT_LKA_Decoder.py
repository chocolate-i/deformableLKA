...
from networks.DAT.dat_4out import DAT
#from DAT.dat_4out import DAT



class DATLKAFormer(nn.Module):
    def __init__(self, num_classes=9, head_count=1, token_mlp_mode="mix_skip"):
        super().__init__()

        # Encoder
        self.backbone = DAT(n_class=num_classes, img_size=224)
    ...
    def forward(self, x):
        ...
        tmp_3 = self.decoder_3(out[3].permute(0, 2, 3, 1).view(b, -1, c))
        tmp_2 = self.decoder_2(tmp_3, out[2].permute(0, 2, 3, 1))
        tmp_1 = self.decoder_1(tmp_2, out[1].permute(0, 2, 3, 1))
        tmp_0 = self.decoder_0(tmp_1, out[0].permute(0, 2, 3, 1))
